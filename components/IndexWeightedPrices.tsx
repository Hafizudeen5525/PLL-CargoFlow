                            ...
                            <div className="mt-2">
                                <div className="flex items-baseline gap-2">
                                    <div className="text-xl font-mono font-bold text-slate-800">
                                        {hasPrice ? `$${res.weightedPrice.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 3 })}` : 'N/A'}
                                    </div>
                                    {hasPrice && Math.abs(delta) > 0.0001 && (
                                        <div className={`flex items-center gap-0.5 text-[10px] font-bold ${isPositive ? 'text-emerald-600' : 'text-rose-600'}`}>
                                            {isPositive ? '▲' : '▼'}
                                            <span>{Math.abs(delta).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 3 })}</span>
                                        </div>
                                    )}
                                </div>
                                        ...
                                        {selectedIdxDetail.breakdown.map((row) => {
                                            ...
                                            return (
                                                <tr key={row.month} className="hover:bg-slate-50 group">
                                                    ...
                                                    <td className="px-4 py-3 text-right font-mono text-slate-400">${row.baselinePrice.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 3 })}</td>
                                                    <td className="px-4 py-3 text-right font-mono text-slate-800 font-bold group-hover:text-blue-600 transition-colors">${row.price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 3 })}</td>
                                                    <td className={`px-4 py-3 text-right font-mono font-bold ${mDelta > 0.0001 ? 'text-emerald-600' : mDelta < -0.0001 ? 'text-rose-600' : 'text-slate-300'}`}>
                                                        {mDelta > 0.0001 ? '+' : ''}{mDelta.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 3 })}
                                                    </td>
                                ...
                                <div className="flex justify-between items-center text-xs font-bold">
                                    ...
                                    <div className="flex gap-4">
                                        <div className="text-right">
                                            ...
                                            <span className="font-mono text-slate-500">${selectedIdxDetail.baselineWeightedPrice.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 3 })}</span>
                                        </div>
                                        <div className="text-right">
                                            <span className="block text-[9px] text-blue-500 uppercase">Target</span>
                                            <span className="font-mono text-blue-600 text-sm">${selectedIdxDetail.weightedPrice.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 3 })}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
