                        ...
                        <div className="space-y-4 h-full overflow-y-auto custom-scrollbar pr-2 min-h-0">
                            ...
                            {testFormula && (
                                <div className="bg-white border border-slate-200 rounded-xl p-4">
                                    ...
                                    <div className="text-lg sm:text-xl font-mono font-bold text-blue-600">${evaluateFormula(testFormula, undefined, targetDate)?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 3 }) ?? 'Error'}</div>
                                </div>
                            )}
                        </div>
                    )}
                                    ...
                                    {(config.metric === 'Purchase' || config.metric === 'Revenue') && (
                                        <>
                                            <td className="px-6 py-4 text-right font-mono">
                                                <div className="text-slate-700 font-bold">${curr.price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 3 })}</div>
                                                {Math.abs(curr.price - base.price) > 0.0001 && (
                                                    <div className={`text-[9px] font-bold ${curr.price >= base.price ? 'text-emerald-500' : 'text-rose-500'}`}>
                                                        {curr.price >= base.price ? '▲' : '▼'} ${Math.abs(curr.price - base.price).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 3 })}
                                                    </div>
                                                )}
                                            </td>
                                            <td className="px-6 py-4 text-right font-mono">
