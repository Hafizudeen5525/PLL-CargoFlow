...
const HOLIDAY_DEFAULTS: Record<string, string> = {
    // 2025 Standard Market Holidays (US, UK, NL)
    '2025-01-01': "New Year's Day (US/UK/NL)",
    '2025-01-20': "MLK Jr Day (US)",
    '2025-02-17': "Presidents' Day (US)",
    '2025-04-18': "Good Friday (UK/NL)",
    '2025-04-21': "Easter Monday (UK/NL)",
    '2025-04-26': "King's Day (NL)",
    '2025-05-05': "Early May Bank Holiday (UK) / Liberation Day (NL)",
    '2025-05-26': "Memorial Day (US) / Spring Bank Holiday (UK)",
    '2025-05-29': "Ascension Day (NL)",
    '2025-06-09': "Whit Monday (NL)",
    ...
    '2025-12-25': "Christmas Day (US/UK/NL)",
...
export const ExposureView: React.FC<ExposureViewProps> = ({ profiles, onCargoClick, portfolioYear }) => {
    ...
    return (
        <div className="flex-1 flex flex-col min-h-0 gap-6 p-2">
            ...
            <AnimatePresence>
                {showHolidayManager && (
                    <HolidayManager onClose={() => setShowHolidayManager(false)} holidays={holidays} onUpdateHolidays={setHolidays} />
                )}
            </AnimatePresence>
                ...
                <div className="flex-1 overflow-auto bg-slate-50 p-6">
                    <div className="grid grid-cols-7 gap-px bg-slate-200 rounded-2xl overflow-hidden border border-slate-200 shadow-inner">
                        ...
                        {Array.from({ length: days.daysInMonth }).map((_, i) => {
                            ...
                            return (
                                <div key={i} onClick={() => toggleHoliday(ds)} className={`bg-white min-h-[110px] p-2 flex flex-col gap-1 transition-all cursor-pointer hover:bg-indigo-50/50 group ${hName ? 'bg-amber-50/30' : ''}`}>
                                    ...
                                    {hName && <div className="bg-amber-500 text-white text-[9px] px-1.5 py-1 rounded-md font-bold shadow-sm animate-in fade-in zoom-in duration-200 truncate" title={hName}>{hName}</div>}
                                    <div className="flex-1" />
                                    {!hName && !isWeekend && <div className="opacity-0 group-hover:opacity-40 text-[9px] font-bold text-indigo-400 text-center uppercase tracking-tighter">Mark Holiday</div>}
                                </div>
                            );
                        })}
                    </div>
...
const DrillDownModal = ({ cell, onClose, profiles, simDate, onCargoClick, holidays }: { cell: any, onClose: () => void, profiles: CargoProfile[], simDate: number, onCargoClick?: (p: CargoProfile) => void, holidays: Record<string, string> }) => {
    const contributors = useMemo(() => {
        ...
        return results;
    }, [profiles, cell, simDate, holidays]);

    return (
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-sm">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-4 max-h-[70vh] custom-scrollbar">
                    {contributors.map((c: any, i: number) => (
                        <div key={`${c.id}-${c.type}-${c.isTier2}-${i}`} onClick={() => { onCargoClick?.(c.profile); onClose(); }} className="p-5 bg-slate-50 rounded-2xl border border-slate-100 hover:border-indigo-200 hover:bg-indigo-50/20 cursor-pointer transition-all space-y-3 group">
                            <div className="flex justify-between items-start">
                                <div>
                                    <div className="text-sm font-bold text-slate-800 group-hover:text-indigo-600 transition-colors">{c.name} {c.isTier2 && <span className="ml-1 px-1 py-0.5 bg-indigo-100 text-indigo-700 text-[8px] rounded uppercase">Tier 2</span>}</div>
                                ...
                                <div className="text-right">
                                    ...
                                    <div className="flex flex-col items-end gap-1 mt-1">
                                        <div className={`text-[9px] font-black uppercase px-2 py-0.5 rounded ${c.isFixed ? 'bg-slate-200 text-slate-500' : 'bg-amber-100 text-amber-600'}`}>{c.isFixed ? 'FULLY FIXED' : 'PRICING ACTIVE'}</div>
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="h-2 w-full bg-slate-200 rounded-full overflow-hidden flex">
                                    <div style={{ width: `${(1 - c.mult) * 100}%` }} className="h-full bg-slate-300 transition-all duration-300" />
                                    <div style={{ width: `${c.mult * 100}%` }} className={`h-full ${c.type === 'Buy' ? 'bg-gradient-to-r from-rose-400 to-rose-500' : 'bg-gradient-to-r from-indigo-400 to-indigo-500'} transition-all duration-300 relative`}>
                                        {!c.isFixed && <div className="absolute inset-0 bg-white/20 animate-pulse" />}
                                    </div>
                                </div>
                            </div>
                        </div>
