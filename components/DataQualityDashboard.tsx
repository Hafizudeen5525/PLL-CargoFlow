...
const TRMS_PROFILE_FIELDS = [
  { value: 'trmsPurchaseVolMMBtu', label: 'Purchase Volume (MMBtu)', type: 'number', description: 'Physical purchase volume in MMBtu for the strategy' },
  { value: 'trmsSalesVolMMBtu', label: 'Sales Volume (MMBtu)', type: 'number', description: 'Physical sales volume in MMBtu for the strategy' },
  { value: 'trmsPurchaseValue', label: 'Purchase Leg Valuation ($)', type: 'number', description: 'Total purchase leg dollar valuation ($)' },
  { value: 'trmsSalesValue', label: 'Sales Leg Valuation ($)', type: 'number', description: 'Total sales leg dollar valuation ($)' },
...
export const DataQualityDashboard: React.FC<DataQualityDashboardProps> = ({ profiles, trmsData, onEditProfile }) => {
  ...
  const [showColumnReference, setShowColumnReference] = useState(false);
  ...
  const [customRules, setCustomRules] = useState<CustomRule[]>(() => {
    try {
      ...
      if (saved) {
        ...
        const combined = [...updated, ...missingDefaults];

        const uniqueRules: CustomRule[] = [];
  ...
  const allQualityIssues = useMemo(() => {
    ...
    if (trmsData && trmsData.trmsAgg) {
      Object.entries(trmsData.trmsAgg).forEach(([strategyName, data]: [string, any]) => {
        const buyLegs = data.commodityLegs?.filter((l: any) => l.buySell === 'Buy') || [];
        const sellLegs = data.commodityLegs?.filter((l: any) => l.buySell === 'Sell') || [];

        const purchaseVol = buyLegs.reduce((sum: number, l: any) => sum + Math.abs(l.volumeInBaseUom || l.volume || 0), 0) || Math.abs(data.purchaseVolume || data.trmsPurchaseVolMMBtu || 0);
        const salesVol = sellLegs.reduce((sum: number, l: any) => sum + Math.abs(l.volumeInBaseUom || l.volume || 0), 0) || Math.abs(data.salesVolume || data.trmsSalesVolMMBtu || 0);

        const purchaseVal = Math.abs(data.trmsPurchaseValue || data.purchaseCost || data.reconciledPurchaseCost || 0);
        const salesVal = Math.abs(data.trmsSalesValue || data.salesRevenue || data.reconciledSalesRevenue || 0);

        const buyPrice = data.weightedBuyPrice ?? (purchaseVol > 0 ? Math.abs((data.commodityValue || purchaseVal) / purchaseVol) : 0);
        const sellPrice = data.weightedSellPrice ?? (salesVol > 0 ? Math.abs((data.commodityValue || salesVal) / salesVol) : 0);
        ...
        trmsStrategies.push({
          ...
          trmsPurchaseVolMMBtu: purchaseVol,
          trmsSalesVolMMBtu: salesVol,
          trmsPurchaseValue: purchaseVal,
          trmsSalesValue: salesVal,
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleSaveRule} className="flex-1 overflow-y-auto p-6 space-y-4 text-xs font-semibold text-slate-705">
                ...
                {/* Quick Templates & Optional Column Reference Explorer */}
                <div className="bg-slate-50 border border-slate-200/80 p-3 rounded-xl space-y-2.5">
                  {/* Quick Rule Templates Row */}
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <span className="text-[10px] font-black uppercase tracking-wider text-slate-500 flex items-center gap-1">
                      <span>⚡ Quick Rule Templates ({ruleDataset}):</span>
                    </span>
                    <button
                      type="button"
                      onClick={() => setShowColumnReference(!showColumnReference)}
                      className="text-[10px] font-bold text-indigo-600 hover:text-indigo-800 flex items-center gap-1 bg-indigo-50/80 hover:bg-indigo-100/80 px-2.5 py-1 rounded-lg border border-indigo-200/60 transition-all"
                    >
                      <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 10h16M4 14h16M4 18h16" />
                      </svg>
                      <span>{showColumnReference ? 'Hide Column Reference List' : 'View Column Reference List'}</span>
                    </button>
                  </div>
                      ...
                      <>
                        <button
                          ...
                          onClick={() => {
                            ...
                            setRuleDescription('Verifies that TRMS Purchase Volume equals Sales Volume for the strategy.');
                            setRuleCategory('Quantity Validation');
                            setRuleSeverity('error');
                            setRuleIntent('requirement');
                            setRuleField('trmsPurchaseVolMMBtu');
                            setRuleCondition('mathCompare');
                            setRhsType('field');
                            setRhsField('trmsSalesVolMMBtu');
                            setMathOperator('equals');
                            setLhsMultiplier('1');
                            setRhsMultiplier('1');
                            toast.success('Loaded template: Purchase Vol == Sales Vol!');
                          }}
                          className="px-2.5 py-1 bg-white hover:bg-indigo-50 border border-slate-200 hover:border-indigo-200 rounded-lg text-[10px] font-bold text-slate-700 hover:text-indigo-700 transition-all shadow-2xs"
                        >
                  ...
                  {/* Optional Collapsible Column Reference List */}
                  <AnimatePresence>
                    {showColumnReference && (
                      <motion.div
                        ...
                        className="pt-2 border-t border-slate-200/60 overflow-hidden"
                      >
                        <div className="flex items-center justify-between mb-1.5">
                          <span className="text-[10px] font-bold uppercase text-slate-500">
                            Available {ruleDataset} Attributes ({ (ruleDataset === 'TRMS' ? TRMS_PROFILE_FIELDS : CARGO_PROFILE_FIELDS).length })
                          </span>
                          <span className="text-[9px] text-slate-400">Click a column to set as target field</span>
                        </div>
                        <div className="flex flex-wrap gap-1.5 max-h-36 overflow-y-auto p-1.5 bg-white rounded-xl border border-slate-200/60">
                          {(ruleDataset === 'TRMS' ? TRMS_PROFILE_FIELDS : CARGO_PROFILE_FIELDS).map(col => {
                            ...
                            return (
                              <button
                                ...
                                className={`px-2 py-0.5 rounded-md text-[10px] font-bold border transition-all text-left flex items-center gap-1.5 ${
                                  isLhsSelected
