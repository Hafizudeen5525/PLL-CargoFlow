            ...
            <div className="flex-1 p-4">
                <div className="grid grid-cols-7 gap-2 h-full text-center">
                    ...
                    {Array.from({ length: daysInMonth }).map((_, i) => {
                        ...
                        return (
                            <div key={day} className={`border rounded-lg p-2 flex flex-col gap-1 min-h-[100px] ${isToday ? 'border-blue-300 ring-2 ring-blue-100' : 'border-slate-100'}`}>
                                <div className={`text-xs font-medium w-6 h-6 flex items-center justify-center rounded-full mb-1 ${isToday ? 'bg-blue-600 text-white' : 'text-slate-500'}`}>
                                    {day}
                                </div>
                                <div className="flex-1 overflow-y-auto space-y-1 custom-scrollbar">
                                    {dayEvents.map((e, idx) => (
                                        <div 
                                            ...
                                            className={`text-[9px] px-1.5 py-1 rounded truncate border-l-2 cursor-pointer transition-all hover:brightness-95 active:scale-95 shadow-sm font-bold ${
                                                e._evtType === 'load' 
                                                ? 'bg-blue-50 border-blue-500 text-blue-700' 
                                                : 'bg-purple-50 border-purple-500 text-purple-700'
                                            }`}
                                            title={`${e._evtType === 'load' ? 'Load' : 'Del'}: ${e.strategyName} (${e._evtType === 'load' ? e.source : e.buyer})`}
                                        >
                                            {e._evtType === 'load' ? 'L: ' : 'D: '}
                                            {e.strategyName}
                                        </div>
