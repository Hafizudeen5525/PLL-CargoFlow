...
export const CargoList: React.FC<CargoListProps> = ({
    ...
    const handleJarvisImport = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const readFile = (file: File): Promise<CargoProfile[]> => {
                ...
                reader.onload = (evt) => {
                    try {
                        ...
                        const finalProfilesArr = Object.values(mergedData).map(p => {
                            ...
                            // We use a temporary ID here, or if we want to merge globally we could do it later.
                            // For now, let's just generate objects.
        ...
        Promise.all(Array.from(files).map(readFile))
            .then((matrix) => {
                ...
                const gatheredProfiles: CargoProfile[] = [];
                matrix.forEach(arr => gatheredProfiles.push(...arr));

                if (onBulkImport) onBulkImport(gatheredProfiles);
                toast.success(`Imported ${gatheredProfiles.length} strategies from ${files.length} file(s)`, { id: loadingToast });
            })
            .catch(err => {
                console.error(err);
                toast.error('Failed to parse one or more Jarvis Macro workbooks', { id: loadingToast });
            })
            .finally(() => {
    ...
    const handleBulkDelete = () => {
        ...
        if (window.confirm(`Are you sure you want to delete ${selectedIds.size} selected cargo(s)?`)) {
            onBulkDelete(selectedIds);
            setSelectedIds(new Set());
            toast.success(`Deleted ${selectedIds.size} cargos`);
        }
    };
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full">
            <div className="px-4 lg:px-6 py-3 border-b border-slate-200 flex flex-col lg:flex-row justify-between items-start lg:items-center bg-white gap-4">
                ...
                <div className="flex items-center gap-2 overflow-x-auto pb-1 sm:pb-0 w-full lg:w-auto custom-scrollbar">
                    <AnimatePresence>
                        {selectedIds.size > 0 && (
                            <motion.button
                                ...
                                className="whitespace-nowrap text-[10px] sm:text-xs font-bold text-rose-600 bg-rose-50 px-3 py-1.5 rounded-lg border border-rose-100 hover:bg-rose-100 transition-colors flex items-center gap-2 shadow-sm"
                            >
                    ...
                    <input type="file" multiple accept=".xlsm, .xlsx" onChange={handleJarvisImport} className="hidden" ref={fileInputRef} />
                                    ...
                                    >
                                        <div className="p-6">
                                            <div className="flex items-center justify-between mb-6">
                                                <div>
                                                    <h3 className="text-lg font-bold text-slate-800">Export Jarvis</h3>
                                                    <p className="text-xs text-slate-500">Configure your export parameters</p>
                                                </div>
                                                <button
                                                    onClick={() => setIsExportPopoverOpen(false)}
                                            ...
                                            <div className="space-y-5">
                                                <div>
                                                    <label className="block text-sm font-bold text-slate-700 mb-2">Portfolio Year</label>
                                                    <select
                                                        value={exportYear}
                                                        onChange={(e) => setExportYear(e.target.value)}
                                                        className="w-full bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium p-3 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all outline-none"
                                                    >
                                                        {availableExportYears.map(y => <option key={y} value={y}>{y}</option>)}
                                                    </select>
                                                </div>
                                                <div>
                                                    <label className="block text-sm font-bold text-slate-700 mb-2">Portfolio Group</label>
                                                    <select
                                                        value={exportGroup}
                                                ...
                                                <div className="pt-4 flex gap-3">
                                                    ...
                                                    <button
                                                        ...
                                                        className="flex-[2] py-3 bg-indigo-600 text-white text-sm font-bold rounded-xl shadow-lg shadow-indigo-200 hover:bg-indigo-700 hover:shadow-indigo-300 transition-all flex items-center justify-center gap-2"
                                                    >
                                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                                                        Generate XLSM
                                                    </button>
